...
try:
    ...
    db = server.create('twquitom')
except:
    #Caso contrario solo conectarse a la base existente
    db = server['twquitom']
    
#Aqui se define el bounding box con los limites geograficos donde recolectar los tweets
twitterStream.filter(locations=[-78.531761,-0.172519,-78.423042,-0.070324])
#twitterStream.filter(track=['ecuador','fiestasdequito'])
#twitterStream.filter(track=['ecuador','Guillermo Lasso'])
